...
use crate::{
    ...
    schema::group::{DetailGroupPagination, GroupAllResponse, GroupDetailUser},
    settings::get_config,
    AppState,
};

#[sqlx::test]
async fn test_paginate_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    ...
    .await?;
    let mut role_factory = GroupFactory::new();
    role_factory.modified_many(|data, _, _| Group {
        id: data.id,
...
#[sqlx::test]
async fn test_get_all_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
...
#[sqlx::test]
async fn test_dropdown_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
...
#[sqlx::test]
async fn test_get_detail_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
    ...
    .await?;
    let mut role_factory = GroupFactory::new();
    role_factory.modified_one(|data, _| Group {
        id: data.id,
        group_name: data.group_name.clone(),
        description: data.description.clone(),
        is_active: data.is_active,
...
#[sqlx::test]
async fn test_create_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
    ...
    let new_role: Option<(String, Option<String>, Option<bool>)> = sqlx::query_as(
        format!(
            r#"
    SELECT group_name, description, is_active
    FROM {}
    WHERE id = $1"#,
            TABLE_NAME
        )
...
#[sqlx::test]
async fn test_update_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
...
#[sqlx::test]
async fn test_delete_group_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
