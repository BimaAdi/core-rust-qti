...
#[sqlx::test]
async fn test_update_permission_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
    ...
    let resp = cli
        ...
        .query("id", &permission.id.to_string())
        .body_json(&json!({
            "permission_name": "update permission name",
    ...
    // Expect
    resp.assert_status_is_ok();
    let updated_permission: Option<Permission> =
        sqlx::query_as(format!("SELECT * FROM {} WHERE id=$1", TABLE_NAME).as_str())
            .bind(&permission.id)
            .fetch_optional(&mut *db)
            .await?;
    assert!(updated_permission.is_some());
    let updated_permission = updated_permission.unwrap();
    assert_eq!(updated_permission.permission_name, "update permission name");
    assert_eq!(
        updated_permission.description,
        Some("update description".to_string())
    );
    assert_eq!(updated_permission.is_user, Some(false));
    assert_eq!(updated_permission.is_role, Some(false));
    assert_eq!(updated_permission.is_group, Some(false));
    let permision_attribute: Vec<PermissionAttributeList> = sqlx::query_as(
        format!(
            "SELECT * FROM {} WHERE permission_id = $1",
            PERMISSION_ATTRIBUTE_LIST_TABLE_NAME
        )
    ...
    .bind(&updated_permission.id)
    .fetch_all(&mut *db)
    .await?;
    assert_eq!(permision_attribute.len(), 2);
    for item in attributes {
        let permision_attribute: Option<PermissionAttributeList> = sqlx::query_as(
            format!(
                "SELECT * FROM {} WHERE permission_id = $1 AND attribute_id = $2",
                PERMISSION_ATTRIBUTE_LIST_TABLE_NAME
            )
        ...
        .fetch_optional(&mut *db)
        .await?;
        assert!(permision_attribute.is_some());
    }
    Ok(())
}

#[sqlx::test]
async fn test_delete_permission_api(pool: PgPool) -> anyhow::Result<()> {
    // Given
    let mut config = get_config();
    config.prefix = Some("/api".to_string());
    let client = redis::Client::open(config.redis_url.clone()).unwrap();
